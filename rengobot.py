...
async def listen_channel(ctx):
	...
	if not config.is_game_admin(ctx):
		await ctx.send(
			"You don't have permissions for this! Only server administrators can configure bot channels."
		)
		return
	...
	await ctx.send(
		f"✅ RengoBot is now listening in {ctx.channel.mention}! Use `$newgame` to start a game."
	)
...
async def list_channels(ctx):
	...
	await ctx.send(
		f"**Active Rengo Channels in {ctx.guild.name}:**\n" + "\n".join(f"• {m}" for m in mentions)
	)
...
async def help(ctx):
	...
	if not channels.is_permitted(ctx):
		if config.is_game_admin(ctx):
			await ctx.send(
				"ℹ️ RengoBot is not currently active in this channel.\n"
				"Type `$listen` to enable games here, or `$channels` to view active channels."
			)
		return
...
async def votes(ctx):
	...
	vdata = all_votes.get(
		game.game_key,
		{"channel_id": ctx.channel.id, "minutes": 10.0, "deadline": None, "votes": {}},
	)
...
async def newgame(ctx, gametype, *args):
	...
	if not config.is_game_admin(ctx):
		await ctx.send(
			"You don't have permissions for this! Only server administrators or bot admins can start games."
		)
		return

	if not is_valid_mode(gametype):
		await ctx.send(
			"Unrecognized game type! Please try `$newgame <vote/queue/random/teachers/anarchy>`"
		)
		return
...
async def resign(ctx, arg):
	...
	if not config.is_game_admin(ctx):
		await ctx.send(
			"You don't have permissions for this! Only server administrators or bot admins can resign games."
		)
		return

	norm_colour = arg.strip().upper()
	if norm_colour in ["BLACK"]:
		norm_colour = "B"
	if norm_colour in ["WHITE"]:
		norm_colour = "W"
