...
from state import GameState, remove_game, save_game
...
class BaseGameMode:
	...
	async def validate_cooldown(self, ctx, game: GameState) -> tuple[bool, str | None]:
		...
		if game.last_times:
			...
			if datetime.now() - last_time < timedelta(seconds=4):
				...
				dropped_msg = (
					f"Last accepted move: {last_move}. Move dropped by anti-spam."
					if last_move
					else "Move dropped by anti-spam."
				)
	...
	async def on_play(self, ctx, game: GameState, move_str: str) -> None:
		...
		file, msg = format_game_message(
			game.game_key, game, next_player_display=player_text, ping_mention=ping_text
		)
		await ctx.send(content=msg, file=file)
