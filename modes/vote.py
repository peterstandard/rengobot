...
import sgfengine
from config import BLACK_STONE_EMOJI, TIME_FORMAT, WHITE_STONE_EMOJI
from modes.base import BaseGameMode
from state import GameState, load_all_games, save_game
...
class VoteMode(BaseGameMode):
	...
	name: str = "vote"
	description: str = (
		"Collective channel voting! Countdown begins on first vote; highest voted move is played."
	)
	...
	async def on_play(self, ctx, game: GameState, move_str: str) -> None:
		...
		all_votes = load_votes()
		vdata = all_votes.get(
			game_key, {"channel_id": ctx.channel.id, "minutes": 10.0, "deadline": None, "votes": {}}
		)
	...
	async def on_queue(self, ctx, game: GameState) -> None:
		...
		await ctx.send(
			f"This game is in Voting mode! Check standings with `$votes`:\n\n{turn_header}\n\n{format_vote_summary(vdata)}"
		)
	...
	async def on_board(self, ctx, game: GameState) -> None:
		...
		file, msg = format_game_message(
			game.game_key,
			game,
			next_player_display=next_player_text,
			title_override="Current Board State",
		)
		if vdata.get("votes"):
			msg += f"\n\n{format_vote_summary(vdata)}"
		await ctx.send(content=msg, file=file)
