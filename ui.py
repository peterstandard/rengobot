...
import sgfengine
from config import BLACK_STONE_EMOJI, WHITE_STONE_EMOJI
...
def format_game_message(
	...
	if title_override:
		...
		move_colour = (
			f"{BLACK_STONE_EMOJI} Black"
			if info["last_colour"] == "B"
			else f"{WHITE_STONE_EMOJI} White"
		)
		...
		if info["last_move"] == "Pass":
			...
			lines.append(
				f"### Move #{info['move_count']} • {move_colour}{player_str} played `{info['last_move']}`"
			)
	else:
	...
	if info.get("consecutive_passes", 0) >= 2:
		lines.append(
			"⚠️ **Game Over:** Both players passed! Use `$resign <B/W>` to record the winner or `$sgf` for the game record."
		)
